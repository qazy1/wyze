...
ctypes.windll.kernel32.SetConsoleTitleW('Wyze')
token = 'ODM1NTc2NDUyMjQxNTU1NTA1.YIRdNg.dM_wn4n6gwV5cbj3JMzlD87P_D0'
prefix = '/'
...
administrators = [752642277456871426]
chat_channel = 835575569726963805
bots_channel = 835576824339496990
queue = []
...
async def on_member_join(member):
    ...
    await channel.send(f'Welcome to **Wyze**, {member.mention}.\nType `/help` to get started!')
...
async def help(ctx):
    ...
    if ctx.channel.type != discord.ChannelType.private:
        ...
        embed.add_field(name='Twitch Spam', value='`Not Avalible`', inline=True)
        embed.add_field(name='Roblox Followers', value='`Not Avalible`', inline=True)
        embed.add_field(name='Roblox Templates', value='`Not Avalible)`', inline=True)
        await ctx.send(embed=embed)
